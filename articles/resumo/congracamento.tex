\documentclass{gescons}

\genre {Resumo do Biênio}
\author{Amanda Vieira}
\title{Editares no 36º Congraçamento das ICs: Plaquinhas, Sorrisos e~Escrita}

\begin{document}
    \makeentrevistatitle
    %\maketitle

    %\fullwidthimage{fields}{b}

    %\coverart{back/editorial}
    \coverart{../fundo-generico}
    
%    \begin{multicols}{2}

%\begin{center}
%    \includegraphics[width=4cm]{articles/entrevista/mockups/Alexandre-Zas}
%\end{center}

\vspace*{\fill}



No dia 8 de dezembro de 2024, durante o~tradicional \textbf{Congraçamento das ICs} -- evento anual de celebração do voluntariado conscienciológico --, a~Editares marcou presença com uma ação voltada aos \textbf{autores e~futuros autores.}

Com placas instigativas e~frases de compromisso com a~escrita, os voluntários foram convidados a~tirar fotos, simbolizando o~engajamento na tarefa autoral. A~atividade trouxe um clima de \textbf{descontração e~leveza,} ao mesmo tempo em que reforçou a~importância do autoposicionamento proexológico perante a~escrita conscienciológica.

\vspace*{\fill}

\begin{center}
\textbf{Confira algumas fotos!}
\end{center}

  \begin{minipage}[b]{0.32\textwidth}
    \includegraphics[width=\linewidth]{articles/resumo/fotos/materia2/IMG20241208144202.jpg}
  \end{minipage}\hfill
  \begin{minipage}[b]{0.32\textwidth}
    \includegraphics[width=\linewidth]{articles/resumo/fotos/materia2/IMG20241208145756.jpg}
  \end{minipage}\hfill
  \begin{minipage}[b]{0.32\textwidth}
    \includegraphics[width=\linewidth]{articles/resumo/fotos/materia2/IMG20241208144514.jpg}
  \end{minipage}
  
  \begin{center}
    \includegraphics[height=7cm]{articles/resumo/fotos/materia2/IMG20241208144653.jpg}
    \hspace{2.5mm}
    \includegraphics[height=7cm]{articles/resumo/fotos/materia2/IMG20241208155101.jpg}
  \end{center}
  %\centering
  %\begin{minipage}[b]{0.9\textwidth}
  %  \includegraphics[height=7cm]{articles/resumo/fotos/materia2/IMG20241208144653.jpg}
  %%\end{minipage}
  %%\begin{minipage}[b]{0.55\textwidth}
  %  \includegraphics[height=7cm]{articles/resumo/fotos/materia2/IMG20241208155101.jpg}
  %\end{minipage}

  \vspace*{\fill}

\coverart{../fundo-generico}

%{[}MURAL/MOSAICO DE FOTOS{]} Fotos na pasta matéria 2\ldots{} as fotos podem ocupar 2 páginas


\clearpage % força nova página
\coverart{../fundo-generico}
\begin{figure}[p] % o~[p] garante que ocupe uma página só de figuras
  \centering
  \includegraphics[width=\textwidth,height=\paperheight,keepaspectratio]{articles/resumo/fotos/materia2/1}
  \caption*{} % sem legenda
\end{figure}
\clearpage

\clearpage % força nova página
\coverart{../fundo-generico}
\begin{figure}[p] % o~[p] garante que ocupe uma página só de figuras
  \centering
  \includegraphics[width=\textwidth,height=\paperheight,keepaspectratio]{articles/resumo/fotos/materia2/2}
  \caption*{} % sem legenda
\end{figure}
\clearpage

        
%    \end{multicols}
\end{document}
